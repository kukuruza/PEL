...
class Evaluator:
    ...
    def process(self, mo, gt):
        ...
        pred = mo.max(1)[1]
        conf = torch.softmax(mo,
                             dim=1).max(1)[0]  # Compute prediction confidences
    ...
    def evaluate(self, num_classes):
        ...
        err = 100.0 - acc
        macro_f1 = 100.0 * f1_score(self._y_true,
                                    self._y_pred,
                                    average="macro",
        ...
        for label in range(num_classes):
            res = (self._per_class_res[label]
                   if label in self._per_class_res else [])
            correct = sum(res)
            total = len(res)
        ...
        hmean_acc = 100.0 / np.mean(
            [1.0 / (max(acc, 0.001) / 100.0) for acc in cls_accs])

        # Compute geometric mean
        gmean_acc = 100.0 * np.prod([acc / 100.0 for acc in cls_accs
                                     ])**(1.0 / len(cls_accs))
        ...
        print(f"* worst_case_acc: {worst_case_acc:.1f}%\n"
        ...
        if self.many_idxs is not None and self.med_idxs is not None and self.few_idxs is not None:
            ...
            print(
                f"* many: {many_acc:.1f}%  med: {med_acc:.1f}%  few: {few_acc:.1f}%"
            )
